...
![vet shield panel (light theme)](assets/white.jpg)

![vet shield panel (dark theme)](assets/dark.jpg)
...
- **0.3.x (user-side 0.3.2 evaluation → alarm-fatigue rebuild)**:
  - **0.3.1**: config **profiles** (`profile: standard/hardened/paranoid`) — one line instead of a pile of toggles; the defense tier and the runtime guard are bound together (`guard: tier`), so weakening the runtime guard also weakens the tier (no silent mismatch); data-plane runtime-guard toggle.
  - **0.3.2**: static engine **v20** + rule surface extended to **R16–R20** (ghost subpath deps, `!!js` config-patch injection, AGENTS.md instruction injection, full-width typosquat, shell download-and-exec in exec/spawn-family args incl. N2-decoded / array / case-folded / concatenated variants); **macOS native metrics** (ps/lsof + fs watch, T1 darwin watch); `--key=value` CLI parsing; **dual-OS CI gate** — ubuntu coverage ratchet (lines/functions/statements ≥ 85%, branches ≥ 80%) + real macOS ps/lsof integration on GitHub Actions.
  - **0.3.3 (alarm-fatigue rebuild)**: coverage-class boundary alerts are now deduplicated **persistently and statefully** via a `known-boundaries` store (keyed by kind, package, version, capability-diff hash, persisted under `~/.dsh/vet/`): the same version/capabilities never re-alarm, an upgrade that changes capabilities auto-revokes the record and re-alarms once (N6 diff source), `alarmCount` counts only *actionable* risk (yellow/red), official content-trust-anchor C2 notices downgrade to info observations that fold across packages (mergeKey) and stay visible/dismissable, and `~/.dsh/sessions` probe noise (lstat/stat only — writes/deletes still alarm) is silenced. The store **fails open**: a damaged or unwritable store repeats notices rather than going blind.
