...
| D30 | **审计改造成 agent 引导式协议（用户两次纠正方向：不要在 vet 里内置审计工具让用户干等，应由 DSH 里的 agent 按预设协议亲自审查）**。触发：真实第三方插件实测（dsh-find-plugin / dsh-remote）暴露 audit_plugin 4 轮 LLM 模板是"伪 agent"——小包 119s、大包超 10 分钟墙钟、用户看不到过程、静态层已抓到的误报（R9 map.set、R2 require('react')）它也不复核。**纠正**：(1) **删除 vet 内置 LLM 审计架构**（orchestrator/prompts/parse/route/session-log + audit_plugin 工具 + audit 配置项 + session-events 声明 + scorecard 的 llm 段 + index.ts 的 llm/sessions/sessionPersistence inject）——vet 不替 agent 调查；(2) 新增 **AUDIT_PROTOCOL.md**（随包分发）：预设审查步骤协议（scan_plugin 拿静态判据 → read 通读清单 → 逐条核实 findings → 按能力面深挖[网络/文件/进程/凭据/库语义如 ssh2 hostVerifier] → 用系统 write 工具把健康档案手写落盘 ~/.dsh/vet/audits/<plugin>-<ver>-<ts>.md），结论判据表 + 职责边界（vet 只扫描/监控/约定目录，不装不卸不替下结论）；(3) **静态层误报修复（真实插件验证）**：R9 map.set/+=/Promise.all 对 for-of/for-in 有界遍历不再报（循环次数受集合大小约束，非无界增长）→ find-plugin 87→100；R2 识别客户端加载器 factory 形参 require（window.__ModuleLoader__.load({ factory: (require) => }) 是 DSH bundle 标准写法）降为 info → remote 84→100；(4) **缓存失效 bug**：scanner 缓存 key 含 ENGINE_VERSION，规则代码改动不 bump 版本会命中旧结果（改规则必须 bump，static-v1→v3，补负例强校验 noRules 断言） | 真实插件端到端：find-plugin clean/100、remote clean/100（修复前 87/84）；agent 按协议审查 remote 落盘档案（含 hostVerifier MITM、密码明文、HTTP 无鉴权 3 项）；159 tests 全绿 |
